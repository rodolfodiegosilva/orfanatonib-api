...
// Função para buscar usuários com role 'leader'
async function getLeaderUsers(accessToken) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'localhost',
      port: 3000,
      path: '/users?role=leader&limit=100',
      method: 'GET',
      headers: {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ...
          const errorBody = JSON.parse(data);
          console.error('❌ Erro ao buscar usuários leaders:', errorBody);
          reject(new Error('Failed to fetch leader users'));
        }
      });
    });
...
// Função para listar leader profiles existentes
...
async function main() {
  ...
  try {
    ...
    // Buscar usuários com role 'leader'
    console.log('\n👨‍💼 Buscando usuários com role "leader"...');
    const leaderUsers = await getLeaderUsers(accessToken);
    ...
    console.log(`✅ Encontrados ${leaderUsers.length} usuários com role "leader"`);
